...
import type { Abilitazione, CampoTicket, ComunicazioneOnboarding, Piattaforma, TipoAbilitazione, TipologicaCampoDinamico } from "../../types/type"
...
function rinumeraCampi(campi: CampoTicket[]): CampoTicket[] {
    return campi.map((c, index) => ({ ...c, order: index + 1 }))
}
...
export function AbilitazioneForm({ piattaforma, initial, tipoBloccato, tipologiche, processi, onCancel, onNew, onSave }: AbilitazioneFormProps) {
    ...
    function salvaCampo(campo: CampoTicket) {
        setCampi((prev) => {
            ...
            let modificato: CampoTicket[]
            if (esiste) {
                modificato = prev.map((c) => (c.id === campo.id ? campo : c))
            } else {
                const newId = nextId(prev)
                modificato = [...prev, { ...campo, id: newId }]
            }
            return rinumeraCampi(modificato)
        })
        setCampoDraft(null)
    }
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {tipoBloccato && (
                    <div className="mt-3 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                        ...
                        <span>
                            {tipoBloccato === "TICKET" ? Constants.abilitazione.TIPO_BLOCCATO_TICKET : Constants.abilitazione.TIPO_BLOCCATO_VERTICALE}
                        </span>
                    </div>
                )}
            ...
            {tipo === "VERTICALE" && (
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-4">
                        ...
                        <select
                            className={INPUT_CLS}
                            value={processoVerticale}
                            onChange={(e) => setProcessoVerticale(e.target.value)}>
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-6">
                        <div className="flex items-center justify-between">
                            ...
                            <Button
                                variant="secondary"
                                onClick={() => setCampoDraft({ ...emptyCampoTicket })}>
                                {Constants.abilitazione.AGGIUNGI_CHIAVE}
                            </Button>
                        </div>
                        <div className="mt-3">
                            <CampiTicketTable
                                ...
                                onDelete={(c) => {
                                    const updated = campi.filter((x) => x.id !== c.id)
                                    setCampi(rinumeraCampi(updated))
                                }}
                            />
                        </div>
                    </div>
