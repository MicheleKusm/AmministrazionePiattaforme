            ...
            <table className="min-w-full">
                ...
                <tbody>
                    {gruppi.map((gruppo, idx) => (
                        <tr key={gruppo.id ?? idx} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                            <td className={`${TD} font-semibold text-gray-900`}>{gruppo.nome}</td>
                            <td className={TD}>{gruppo.descrizione}</td>
                            <td className={TD}><Badge tone="gray">{gruppo.ruoliIds.length} ruoli</Badge></td>
                            <td className={TD}>
                                <div className="flex gap-2">
                                    <button type="button" aria-label="Modifica" onClick={() => onEdit(gruppo)} className={ICON_BTN}>
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                            <path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" />
                                        </svg>
                                    </button>
                                    <button type="button" aria-label="Elimina" onClick={() => onDelete(gruppo)} className={`${ICON_BTN} hover:border-red-300 hover:text-red-600`}>
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
